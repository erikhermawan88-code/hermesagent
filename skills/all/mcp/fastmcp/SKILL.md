...
## Real-World Example: Roti Bakar 88 MCP Server

A working FastMCP server that manages website menu data via natural language.

**Reference implementation:** `references/rotibakar88-mcp.md`

Key patterns:
- Data stored in `data/menu.json` with structure `{"menu": [...]}` (nested, not flat array)
- All tools return structured JSON with `success` flag
- Auto-backup on every write to `data/backups/menu_YYYY-MM-DD_HHMMSS.json`
- Fields: `name`, `desc`, `price` (display string), `price_val` (numeric), `image`, `badge`, `wa_link`
- Tested via direct Python import before trying FastMCP CLI

**Hermes integration (required for actual use):**
After building and testing the server, add to `~/.hermes/config.yaml` under `mcpServers` to expose tools to Hermes Agent. See `native-mcp` skill for config format.

## Pitfalls

### JSON structure mismatch
Many JSON data files use nested structures like `{"menu": [...]}` instead of flat arrays. Tools that iterate `for i in data` break because `data` is a dict, not a list.

**Fix:** Inspect JSON structure first. Use `get_menu_items()` helper:
```python
def get_menu_items():
    data = load_menu()
    return data['menu'] if 'menu' in data else data  # handle both nested and flat
```

### price vs price_val
Menu items often store display price (`"Rp 12,000"`) separately from numeric value (`12000`) for sorting/comparison. Update both fields when changing price.

### Hermes not connected
Building the server is step 1. For Hermes to actually use the tools via natural language, you MUST add the server to `~/.hermes/config.yaml`. See `native-mcp` skill.
