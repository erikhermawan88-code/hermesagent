...
### Gateway Crash: Voice Message → ImportError

A known crash pattern occurs when the gateway receives a voice message and the transcription pipeline tries to import from `tool_backend_helpers`:
...
**Symptoms:** Gateway dies 1-2 minutes after a user sends a voice message. Logs show the voice message was cached/accepted, then the crash traceback, then `SIGTERM` shutdown.

**Workaround (manual restart):**
```bash
cd ~/.hermes/hermes-agent && source venv/bin/activate && hermes gateway run &>/tmp/gateway-restart.log &
```

Wait ~5s, then verify: `ps aux | grep "hermes gateway" | grep -v grep`

**Why manual:** The gateway's own watchdog doesn't restart itself on crash — systemd restart or manual relaunch is required. This may be addressed in a future version.
