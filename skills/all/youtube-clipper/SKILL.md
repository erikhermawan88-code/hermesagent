...
### 5. YouTube Auth & Cookie Handling
**重要**: See `references/youtube-auth-cookies.md` for handling HTTP 403 / bot protection errors when downloading YouTube videos.

### 6. Download Alternatives
**重要**: See `references/youtube-download-alternatives.md` for tested alternatives and anti-detection strategies. Only yt-dlp is reliable — youtube-dl and gallery-dl are NOT viable substitutes.

### 7. yt-dlp Critical Flag (May 2026)
The `--extractor-args "youtube:player_client=web"` flag is **essential** for reliable downloads. Without it, YouTube may return errors or use suboptimal formats. Always include this when downloading from YouTube directly.
