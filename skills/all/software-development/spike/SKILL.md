...
# Import check — critical for gunicorn
...
# Register + Login flow (with cookie jar)
curl -c /tmp/s.txt -X POST -d "email=...&password=..." http://localhost:5000/auth/register
curl -b /tmp/s.txt -X POST -d "email=test@example.com&password=test1234" http://localhost:5000/auth/login
curl -b /tmp/s.txt http://localhost:5000/dashboard/ | grep "<title>"
```

For common Flask MVP pitfalls documented during build (gunicorn factory, Flask-Login, url_for, etc.), see `references/flask-mvp-pitfalls.md`.

## YouTube Tutorial → Spike Pattern

When the upstream source is a video (no repo, no docs to read):

**Spike 0 — Extract content** (always run first):
```bash
...
```
Parse the `.vtt` to understand what the video covers. This IS the research phase.

**Spike 1 — Spec from video**: Convert video concepts → `SPEC.md` before writing any code.
**Spike 2 — Prototype structure**: Build the minimal shell and verify it runs before adding features.
**Spike 3 — Feature fill**: Implement feature-by-feature using the video as blueprint. Mock data when APIs aren't available. Swap in real APIs later — same shape, drop-in.

**Mock data trick for prototypes**: When live APIs aren't configured yet, generate realistic data programmatically and structure the backend to return the exact JSON shape the real API would. This makes the swap trivial.

Full session notes: `references/youtube-fullstack-build.md` under `nextjs-tailwind-setup`.
